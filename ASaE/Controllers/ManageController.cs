    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var model = new ManageIndexViewModel
            {
                ...
                Requests = await db.Requests.Where(r => r.Client.Id == user.Id).Where(r => r.Status == RequestStatus.Waiting).ToListAsync() ?? new List<Request> { },
                Contracts = await db.Contracts.Where(c => c.Client.Id == user.Id).ToListAsync() ?? new List<Contract> { }
            };
        ...
        public FileResult GetInstruction()
        {
            return File(System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath + 
                "/Content/instructions/instruction.txt", System.Net.Mime.MediaTypeNames.Text.Plain, "Инструкция.txt");
        }
